...
//
// ⚠️ مصدر الحقيقة الفعلي هو جدول `billing_plans` على كايروس ويب (مشروع B) —
// تُزرَع صفوفه من `apps/web/schema/migrations/2026-09-04-billing-plans-ladder.sql`
// في مستودع الـ POS وتُعدَّل من كونسول المشغّل (تبويب «الخطط»). هذه نسخة ثابتة
// للعرض التسويقي — أي تعديل سعر أو نقل وحدة هناك يجب أن يُحدَّث هنا يدوياً أيضاً،
// وإلا انحرف الموقع عن الفوترة الحقيقية.
//
// سلّم 2026-09-04 (دراسة المنافسين، الجزء الأول §6): 240 / 380 / 540 سنوياً،
// الشاملة تشمل فرعَين و90 ديناراً سنوياً لكل فرع إضافي. رموز الخطط في السحابة:
// rest-basic/biz/full و market-basic/biz/full — المفتاح هنا (`key`) تسويقي فقط.
//
// 2026-09-05 (المرحلة الأولى من دراسة المنافسين): أُضيفت للبزنس «تنبيهات المالك على
// تلغرام» (owner_alerts) و«سجل التدقيق» (audit_log) — مفتاحان في FEATURE_CATALOG تثبّتهما
// (2026-09-09 أُضيف «إقرار ضريبة المبيعات» (vat_return) بنفس الشكل: البزنس والشاملة.)
// صفوف *-basic على false — وللأساسية «شاشة العميل» التي تعمل الآن من المتصفح.
//
// `features` = متاحة فعلاً اليوم **ومُنفَّذة بالخطة**: كل سطر هنا يقابل وحدة يحجبها
// الخادم عن الباقة الأدنى (server/features.js FEATURE_CATALOG) أو سقفاً رقمياً
// (أجهزة/فروع). ما كان يُعرَض كفارق وهو مشترك لكل العملاء (اللوحة السحابية،
// تصدير Excel، «الصلاحيات المتقدمة»، «الوصول المبكر»، «دعم بالأولوية») أُزيل في
// 2026-09-04 — لا نبيع فارقاً لا يفرّقه النظام. `soon` = على خارطة الطريق ولم
// تُطلق بعد، وتُعرض موسومة «قريباً» — لا نَعِد الزائر بما لا يعمل الآن.
//
...
export const PLANS = [
  {
    ...
    features: [
      ...
      "التقارير الأساسية وتصدير Excel",
      "لوحة تحكّم من أي متصفّح — والبيع يكمل بلا إنترنت",
      "شاشة العميل على شاشة ثانية من نفس الجهاز",
      "ربط جوفوترة وتحديثات مجانية ودعم على واتساب",
    ],
    soon: [],
  },
  {
    ...
    features: [
      ...
      "شاشة المطبخ KDS",
      "تطبيق المدير على الجوال وحسابات «عرض فقط»",
      "تنبيهات المالك على تلغرام: ملخص اليوم، عجز الوردية، مخزون منخفض، قفزة إلغاءات",
      "سجل التدقيق: من ألغى وخصم وحذف وغيّر سعراً، ومن أي جهاز، ومتى",
      "إقرار ضريبة المبيعات: المخرجات ناقص المدخلات، ومعه كل فاتورة وراء الرقم — جاهز للمحاسب",
    ],
    soon: [],
  },
  {
    ...
    branches: "فرعان مشمولان",
    // سعر الفرع الإضافي سنوياً — يطابق extra_branch_price_yearly في صف rest-full/market-full.
    extraBranchPrice: 90,
    features: [
      "كل ما في البزنس",
      "تعدد الفروع: مخزون مستقل وتقارير مجمّعة، +90 ديناراً سنوياً لكل فرع إضافي",
      "مستودع مركزي وتحويلات مخزون بين المواقع",
      "تطبيق الويتر على التابلت",
      "التقارير المتقدمة: الذروة والربحية والدوران والتوقّع",
    ],
...
export const WA_MESSAGES = {
  ...
  online: "مرحباً 👋 بدي أفعّل كاشير كايروس الأونلاين (بلا قاعدة محلّية) لمحلّي.",
...
// ── الكاشير الأونلاين (2026-09-11) ────────────────────────────────────────────
// منتج ثانٍ موازٍ للخفيف، لا بديل عنه. الفرق الذي يهمّ المشتري: الخفيف يملك نسخة
// محلّية من البيانات فيبيع بلا إنترنت ثم يزامن، والأونلاين لا يملك نسخة أصلاً —
// ولهذا لا مزامنة فيه أساساً، ولا يبيع عند انقطاع الخط. اختيار بين مقايضتين، لا
// بين قديم وجديد.
//
// ⚠️ الرابطان أدناه لا يعملان حتى يرفع المالك الملفين ويجهّز قناة التحديث
// (online-updates.kairos-pos.com + دلو R2). البطاقتان في صفحة التحميل تعرضان
// اليوم زرّ «اطلب التفعيل» عبر واتساب بدل رابط مباشر — والميزة نفسها مطفأة لكل
// مستأجر حتى يفعّلها المشغّل، فرابط عامّ الآن يعطي 404 ويَعِد بما لا يُسلَّم.
// بعد الرفع: بدّل الزرّين بهذين الثابتين، وأضف رقم الإصدار كما في الخفيف.
